...
namespace BartKFSentinels.Alaalu
{
    public class TheHeartOfTheWorldCardController : CardController
    {
        ...
        {
            base.AddTriggers();
            // "The first time a target would be put into a trash each turn, shuffle it into its deck instead."
            AddTrigger<MoveCardAction>((MoveCardAction mca) => !HasBeenSetToTrueThisTurn(OncePerTurn) && mca.CardToMove.IsTarget && mca.Destination.IsTrash, ShuffleInsteadResponse, TriggerType.ShuffleCardIntoDeck, TriggerTiming.Before);
