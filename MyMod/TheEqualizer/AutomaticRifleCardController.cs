...
namespace BartKFSentinels.TheEqualizer
{
    public class AutomaticRifleCardController : MunitionCardController
    {
            ...
            : base(card, turnTakerController)
        {
            ...
            SpecialStringMaker.ShowHighestHP(numberOfTargets: () => H - 1, cardCriteria: new LinqCardCriteria((Card c) => IsHeroTarget(c) && !IsMarked(c), "non-[b][i]Marked[/i][/b] hero", singular: "target", plural: "targets"));
        }
        ...
        public override IEnumerator SalvoAttack()
        {
            ...
            IEnumerator shootOtherCoroutine = DealDamageToHighestHP(CharacterCard, 1, (Card c) => IsHeroTarget(c) && !IsMarked(c), (Card c) => 3, DamageType.Projectile, numberOfTargets: () => H - 1);
            if (base.UseUnityCoroutines)
            {
