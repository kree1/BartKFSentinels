    ...
    public class MindTheGapCardController : CardController
    {
        ...
        public override IEnumerator Play()
        {
            if (GameController.FindCardsWhere((Card c) => c.IsInPlay && IsHero(c) && !c.IsCharacter).Any())
            {
                ...
                //SelectTurnTakersDecision destroyOrder = new SelectTurnTakersDecision(base.GameController, this.DecisionMaker, new LinqTurnTakerCriteria((TurnTaker tt) => IsHero(tt)), SelectionType.DestroyCard, isOptional: true, allowAutoDecide: true, cardSource: GetCardSource());
